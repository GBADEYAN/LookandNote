...
require("../../Metier/Gestion_stat.php");
...
//149Gestion ami
	if(isset($_POST["rech_ami"])){//Rech ami potentiel	
		$ppp=aff_ami_pot($_POST["rech_ami"]);
		$page="ami.php";
	}
	if(isset($_POST["ajout_ami"])){//Ajout d'ami	
		ajout_ami($_SESSION["id"],$_POST["id_ami"]);
		$page="ami.php";
	}
	if(isset($_POST["supp_ami"])){//Supp d'ami	
		supp_ami($_SESSION["id"],$_POST["id_ami"]);
		$page="ami.php";
	}
    if($page=="ami.php"){
...
//160Gestion des files
	if($page=="accueil_util.php"){
		...
		if(isset($_POST["lien_photo"])){
			...
			$num_photo=numph_lien($_POST["lien_photo"]);
			$N=aff_stat_photo($num_photo);
			$page="note.php";
		}
	}
	
//177--Publication de photo (insertion)
	...
	if(isset($_POST["submit_c2"])){ //ajout critere
...
//217Gestion mon profil
	if($page=="profil.php"){
	    $stat=aff_stat_profil($_SESSION["id"]);
		$pp=aff_photo_perso($_SESSION["id"]); 
		$nd=aff_note($_SESSION["id"],"donnee"); 
		$nr=aff_note($_SESSION["id"],"recue");
...
//Gestion page evaluation
    if($page=="evaluation_donnee.php"){
        $nd=aff_detail_note($_SESSION["id"],"donnee");
    }
    if($page=="evaluation_recue.php"){
        $nr=aff_detail_note($_SESSION["id"],"recue");
    }
